...
func NewWIF(privKey *hdfec.PrivateKey, net *chaincfg.Params, compress bool) (*WIF, error) {
	if net == nil {
...
func DecodeWIF(wif string) (*WIF, error) {
	...
	switch decodedLen {
	case 1 + hdfec.PrivKeyBytesLen + 1 + 4:
		if decoded[33] != compressMagic {
			return nil, ErrMalformedPrivateKey
		}
		compress = true
	case 1 + hdfec.PrivKeyBytesLen + 4:
		compress = false
	default:
		return nil, ErrMalformedPrivateKey
	}
	...
	var tosum []byte
	if compress {
		tosum = decoded[:1+hdfec.PrivKeyBytesLen+1]
	} else {
		tosum = decoded[:1+hdfec.PrivKeyBytesLen]
	}
	...
	netID := decoded[0]
	privKeyBytes := decoded[1 : 1+hdfec.PrivKeyBytesLen]
	privKey, _ := hdfec.PrivKeyFromBytes(hdfec.S256(), privKeyBytes)
...
func (w *WIF) String() string {
	...
	// bytes of checksum.
	encodeLen := 1 + hdfec.PrivKeyBytesLen + 4
	if w.CompressPubKey {
		encodeLen++
	}
	...
	a = paddedAppend(hdfec.PrivKeyBytesLen, a, w.PrivKey.D.Bytes())
	if w.CompressPubKey {
...
func (w *WIF) SerializePubKey() []byte {
	pk := (*hdfec.PublicKey)(&w.PrivKey.PublicKey)
	if w.CompressPubKey {
